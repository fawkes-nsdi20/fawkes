...
make_fawkes_default() {
    ...
    if [[ -z "$main_file" ]]; then
        echo "Failed to grep the main file for $fawkes_dir"
        return 10
    fi
...
prepare_page () {
    ...
    rc=$?
    if [[ $rc != 0 ]]; then
        echo "[Error] Failed to generate static template for $page_name"
        cd ../
        return $rc
    fi
    ...
    rc=$?
    if [[ $rc != 0 ]]; then
        echo "[Error] Failed to generate dynamic patch for $page_name"
        cd ../
        return $rc
    fi
    ...
    # Creates the Fawkes version (Mahimahi dir) of target HTML
    make_fawkes_default
    rc=$?
    if [[ $rc != 0 ]]; then
        echo "[Error] Failed to create replay directories for $page_name."
        cd "$last_wd"
        return $rc
    fi
    cd "$last_wd"
